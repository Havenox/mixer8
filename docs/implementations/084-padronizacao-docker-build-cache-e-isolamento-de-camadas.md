# 084 - Infra: Padronização de Docker Build Cache, Playwright Oficial (.NET 10), FFmpeg Estático, Espelho UFSCar e Pip Resiliente
...
Durante o fluxo de Continuous Delivery e manutenção em produção, alterações pontuais de código-fonte resultavam em tempos de compilação excessivos e falhas de runtime e timeout de rede no servidor:
1. **Compilação Manual Excessiva e Incompatibilidade no Extrator:** O `mixer8-extractor` instalava centenas de pacotes Linux para rodar o Chrome. Ao adotar a imagem Playwright da Microsoft, o runtime padrão embutido era o .NET 8.0, gerando falha de inicialização (`Microsoft.NETCore.App version 10.0.0 missing`).
...
1. **Playwright Oficial com .NET 10 Injetado:** Utilização da imagem `mcr.microsoft.com/playwright/dotnet:v1.49.0-noble` com injeção direta do runtime .NET 10 via `COPY --from=mcr.microsoft.com/dotnet/runtime:10.0 /usr/share/dotnet /usr/share/dotnet`. Todas as 120+ libs do Chrome e Node são preservadas com Zero `apt-get`.
...
| Microsserviço | Método Anterior | Novo Método Otimizado | Tempo de Rebuild | Status Local |
| :--- | :--- | :--- | :--- | :--- |
| **`mixer8-extractor`** | 100+ debs manuais (.NET 8 clash) | Imagem Oficial Playwright + .NET 10 | **~3s** | ✅ UP (Porta 5010) |
| **`mixer8-waveformer`** | apt-get ffmpeg (217 pacotes) | Binário estático `mwader/static-ffmpeg` | **0.4s** (0 apt-get) | ✅ UP (Worker ativo) |
| **`mixer8-api`** | apt-get ffmpeg (217 pacotes) | Binário estático `mwader/static-ffmpeg` | **0.4s** (0 apt-get) | ✅ UP (Porta 5000) |
| **`mixer8-downloader`** | apt internacional lento (35m) | Espelho UFSCar + FFmpeg estático + Pip blindado | **18.4s** (primeiro) / **~3s** (rebuilds) | ✅ UP (Worker ativo) |
| **`mixer8-app`** | NGINX + Multi-stage | Build Vite cacheado | **0.0s (CACHED)** / **~2s** | ✅ UP (Porta 3000) |
