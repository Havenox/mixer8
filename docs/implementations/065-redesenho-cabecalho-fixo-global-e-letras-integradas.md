...
* **`DawView.tsx`**: Removido o bloqueio de tela para telas menores que 768px (`isDesktopOrTablet`). Implementado o estado reativo `isSidebarCollapsed` e o botão de toggle na régua (`PanelLeftOpen` / `PanelLeftClose`) que encolhe os controles de pistas para `48px` no celular, exibindo abreviações de 3 letras e botões M/S compactos, priorizando a visibilidade das formas de onda.
