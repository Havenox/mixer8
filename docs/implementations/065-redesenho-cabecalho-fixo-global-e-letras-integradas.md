...
* **`MesaPlayer.tsx`**: Simplificados os rótulos de botões do player expandido mobile para **`DAW`**, **`Mixer`** e **`Letras`**. O acionamento de qualquer utilidade ou do player em tela cheia agora utiliza a maquina de estados unificada `ActiveOverlayType` (`'none' | 'daw' | 'lyrics' | 'mixer' | 'player'`), garantindo alternância mutuamente exclusiva.
