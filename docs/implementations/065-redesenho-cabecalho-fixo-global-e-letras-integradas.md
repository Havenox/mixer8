...
- **Suporte Total a Mobile (Responsividade Dinâmica)**: O cabeçalho fixo global foi reconfigurado para se adaptar a smartphones. As informações de faixa (capa, título, artista) são ocultadas em telas menores que `768px` (`md`), pois já são exibidas no player de rodapé. Isso abre espaço total para as caixas de controle (Zoom, Cifra, Acorde, Tom, BPM e Fechar). O layout mobile é alinhado à direita (`justify-end`) com espaçamento extremamente curto (`gap-1`) para evitar grandes vazios visuais. As caixas ganharam larguras ultra-compactas (Zoom a 90px, Cifra a 50px, Acorde a 56px, Tom a 88px, BPM a 92px), botões reduzidos (`w-[22px]`) com ícones menores (`w-2.5`), e o botão de fechar (X) retrai a `w-0` quando oculto no celular (mantendo `w-10` no desktop para evitar layout shifts). A altura do cabeçalho reduz de `72px` para `56px` no mobile.

## 🛠️ Implementação Técnica

### Frontend (`mixer8-app`)
* **`GlobalTopHeader.tsx`**: Centralizados metadados, acorde reativo, transposição, BPM e controles de Zoom. O bloco de Zoom agora renderiza no lado direito (à esquerda do bloco de Acorde) para manter estabilidade visual. Adicionado o botão Close (X) em contêiner de largura física invariável no desktop para evitar layout shifts, mas que se retrai no mobile para economizar espaço. Reestilizados os textos de topo das caixas e aplicada a reconfiguração de proporções, alinhamento compacto à direita e ocultações reativas para dispositivos móveis (mobile).
