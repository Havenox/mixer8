...
* **`DawView.tsx`**: Removido o bloqueio de tela para telas menores que 768px (`isDesktopOrTablet`). Implementado o estado reativo `isSidebarCollapsed` e o botão de toggle na régua (`PanelLeftOpen` / `PanelLeftClose`) que encolhe os controles de pistas para `48px` no celular. Corrigida a renderização de waveforms substituindo a multiplicação direta de picos por uma fórmula de amplitude normalizada (`absVal / 100.0 * height * 0.42`) com preenchimento contínuo de polígono envelope, restaurando a dinâmica orgânica de áudio sem blocos sólidos estourados.
* **`MobileMixerOverlay.tsx`**: [NOVO] Componente dedicado para o Mixer de Som Mobile, renderizado como um overlay/modal próprio (`activeOverlay === 'mixer'`) entre o `GlobalTopHeader` e o player de rodapé, com sliders de volume de stems em tempo real e presets rápidos (Voz, Sem Voz, Instru., Reset).
* **`MesaPlayer.tsx`**: Simplificados os rótulos de botões do player expandido mobile para **`DAW`**, **`Mixer`** e **`Letras`**. O acionamento de qualquer utilidade ou do player em tela cheia agora utiliza a maquina de estados unificada `ActiveOverlayType` (`'none' | 'daw' | 'lyrics' | 'mixer' | 'player'`), garantindo alternância mutuamente exclusiva (abrir um fecha automaticamente o outro).
