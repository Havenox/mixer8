...
54. **Extrator: Desacoplamento Total do Download e Motor Nativo com Watchdog de Inatividade (.NET Heartbeat)**:
    * **Bypass de Download no Navegador:** O navegador Chromium/Playwright foi completamente desacoplado do ato de gravar arquivos em disco. O Playwright é empregado estritamente para navegar na DAW e capturar a URL pré-assinada do Google Cloud Storage (`download.Url`). Assim que a URL é obtida, o Chromium é fechado imediatamente (`await context.DisposeAsync()`), liberando instantaneamente mais de 1.5 GB de buffers de áudio alocados pela Web Audio API e eliminando de forma definitiva a race condition de `TargetClosedException` causada pelo teardown de iframes no motor do Chromium.
    * **Motor de Download Nativo com Watchdog (`DownloadFileWithHeartbeatAsync`):** O download do arquivo ZIP de stems passa a ser executado 100% pelo .NET via `SocketsHttpHandler` em blocos de 80 KB. Cada leitura de bloco é protegida por um `CancellationTokenSource` com watchdog de inatividade de 45 segundos e timeout global de 10 minutos. Se a conexão estagnar (socket half-open / packet drop), o download é abortado em 45 segundos em vez de congelar a fila por dias.
    * **Supressão de Conflito de Autenticação:** Eliminada a injeção do cabeçalho `Authorization: Bearer <moises_token>` em requisições diretas a URLs pré-assinadas da Google Cloud Storage (`storage.googleapis.com`), evitando rejeições e encerramentos silenciosos de conexão.
    * **Resiliência e Continuidade da Fila:** Qualquer falha dispara retentativas automáticas (até 3 tentativas com backoff) ou lança exceção controlada, registrando em `SystemEvents`, atualizando a track no PostgreSQL e garantindo que o worker nunca pare de processar a fila.
55. **Extrator: Expurgo Automático de Travas Órfãs do Chromium (SingletonLock) em Volumes Docker**:
    * **Desafio do Process Singleton:** Em contêineres Docker reiniciados abruptamente (`docker restart`, interrupções manuais ou quedas de host), o Chromium mantém travas físicas e symlinks no volume compartilhado (`user_profile`), especificamente `SingletonLock`, `SingletonCookie` e `SingletonSocket`. Ao subir um novo contêiner com hostname/PID diferente, o Chromium aborta a inicialização com `The profile appears to be in use by another Google Chrome process...`, disparando `TargetClosedException` no Playwright.
    * **Rotina de Purga Preventiva (`PurgeStaleChromiumLocks`):** Implementada varredura pré-lançamento no `Worker.cs` que inspeciona o diretório `user_profile` e remove defensivamente todos os arquivos e symlinks correspondentes a `Singleton*` e `lockfile*` antes de invocar `LaunchPersistentContextAsync`. Garante autocura completa do contêiner sem necessidade de expurgo manual de arquivos no servidor.
56. **Extrator: Otimização de Throughput com HTTP/1.1 e Buffers de 1 MB para Download de Stems**:
    * **Eliminação do Gargalo de Janela HTTP/2:** O `DownloadFileWithHeartbeatAsync` no `Worker.cs` foi reconfigurado para forçar estritamente `HttpVersion.Version11` (`HttpVersionPolicy.RequestVersionExact`). Isso remove o estrangulamento da janela padrão de 64 KB do HTTP/2 (`InitialHttp2StreamWindowSize`), permitindo que o kernel Linux utilize TCP Window Scaling dinâmico de múltiplos megabytes, saturando 100% da banda disponível (30+ Mbps) e reduzindo o tempo de download de 182 MB de 35 minutos para < 50 segundos.
    * **Supressão de PING Frames Agressivos:** Removidas as propriedades `KeepAlivePingDelay = 15s` e `KeepAlivePingTimeout = 10s` do handler de download, eliminando cancelamentos prematuros de socket (`SocketException 125`) causados por atrasos de resposta PING ACK na CDN do Google Storage sob fluxo pesado de dados.
    * **Expansão de Buffers de I/O para 1 MB:** Elevado o buffer de leitura do socket e gravação de disco (`FileStream`) de 80 KB para 1.048.576 bytes (1 MB), minimizando a sobrecarga de context switching e syscalls de CPU.
57. **Infraestrutura: Calibração de MTU da Rede Docker (1420) para Túnel WireGuard (CGNAT Bypass)**:
    * **O Desafio do PMTU Black Hole:** Servidor de produção opera em homelab atrás de CGNAT, utilizando túnel WireGuard (`wg0`, IP `10.8.0.2`) conectado a VPS pública para entrada e saída de tráfego. O túnel consome 80 bytes de cabeçalho criptográfico, exigindo MTU de 1420 bytes. Redes bridge do Docker eram provisionadas com MTU padrão de 1500 bytes. Ao baixar stems do Google Cloud Storage, pacotes de 1500 bytes eram descartados silenciosamente pelo WireGuard, provocando perda massiva de pacotes, colapso de TCP para 100 KB/s e timeouts de socket.
    * **A Loteria Anycast do Google:** Explicada a oscilação onde downloads funcionavam a 5 MB/s (nós com RFC 4821 / PLPMTUD que reduziam o tamanho dos pacotes dinamicamente) e outros travavam a 100 KB/s (nós com RFC 1191 dependentes de ICMP bloqueado por CGNAT).
    * **Padronização Declarativa:** Configurada a rede `default` no `docker-compose.yml` com `com.docker.network.driver.mtu: "${DOCKER_NETWORK_MTU:-1420}"` e documentada no `.env.example`. Força os contêineres a negociarem `MSS = 1380 bytes` logo no handshake TCP, garantindo que 100% dos pacotes passem pelo túnel sem descarte e fixando a taxa em 4,5–5 MB/s em todas as músicas.
