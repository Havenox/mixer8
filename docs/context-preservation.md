...
52. **Padronização de Docker Build Cache, Playwright Oficial, FFmpeg Estático, Espelho UFSCar e Pip Resiliente**:
    ...
    * **Migração do Extrator para Imagem Oficial da Microsoft + .NET 10:** No [mixer8-extractor/Dockerfile](file:///g:/DEV/mixer8/mixer8-extractor/Dockerfile), o runtime foi migrado para `mcr.microsoft.com/playwright/dotnet:v1.49.0-noble` com injeção do .NET 10.0 runtime (`/usr/share/dotnet`), eliminando 100% de etapas `apt-get` e compilações manuais de S.O., mantendo Chrome e Node pré-instalados.
