...
52. **Padronização de Docker Build Cache, Imagem Oficial Playwright e Correção de Timeout IPv6**:
    ...
    * **Eliminação de Timeout de IPv6 no APT:** Adicionada a diretiva `Acquire::ForceIPv4 "true"` nos containers que utilizam `apt-get` (`api`, `downloader`, `waveformer`), eliminando o hang de 30s por pacote gerado por falhas de rota IPv6 nas bridges Docker no Linux.
