...
* **Componentização Unificada de Layouts (Grade/Lista/Lista Compacta)**: Para eliminar redundância de código e garantir uma experiência de visualização homogênea, todas as listagens de faixas e playlists foram delegadas aos componentes dedicados `TrackListing` e `PlaylistListing` (com chave `mixer8:layout-preference`). Adicionalmente, as prateleiras de destaque do Explorar foram modularizadas no componente `ExploreShelf`, suportando o modo **Grade** e o novo layout **Lista Compacta** (tiles retangulares horizontais com capas em miniatura que acomodam até 6 elementos alinhados responsivamente em `lg:grid-cols-6` e `md:grid-cols-3`, com persistência sob a chave `mixer8:explore-layout-preference`).
